<template>
  <div class="img_effect clearfix">
    ...
    <ul class="img_operation clearfix">
      ...
      </li><a href="https://meiyoufan.com">blog</a>
    </ul>
  </div>
</template>
        ...
        Promise.all(allImages).then(res => {
          ...
          res.forEach((blob, index) => {
            if (blob) {
              ...
              formData.append('avatars[' + index + '].name', name)
              formData.append('avatars[' + index + '].width', avatar.width)
              formData.append('avatars[' + index + '].height', avatar.height)
              formData.append('avatars[' + index + '].x', avatar.x)
              formData.append('avatars[' + index + '].y', avatar.y)
            } else {
              isValid = false
            }
          })
